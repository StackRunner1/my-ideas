...
def test_process_query_with_tool_call_success(mock_get_client):
    ...
    mock_turn2_response.output = [
        Mock(
            ...
            role="assistant",
            content=[
                Mock(type="output_text", text="You have 3 ideas: Idea 1, Idea 2, Idea 3")
            ]
        )
    ]
...
def test_process_query_sql_validation_blocks_unsafe(mock_get_client):
    ...
    mock_function_call.arguments = '{"sql": "DELETE FROM ideas WHERE id = 1", "explanation": "Deleting idea"}'
...
def test_tool_handler_handles_database_error():
    ...
    result = execute_query_database(
        mock_agent_client,
        sql="SELECT * FROM ideas LIMIT 10",
        explanation="Get ideas"
    )
